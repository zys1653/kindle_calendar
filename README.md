...
**版本 0.1.6：统一微软账户登录、合并等待同步队列，顶部纳入邮箱同步状态。本次按用户要求额外执行仿真与视觉检查；真实邮箱和真机效果待验收。** 不会自动连接设备。首次安装先诊断、校准，再启用完整接管。
...
- 邮箱：设置第 2 页统一登录／补充授权，与待办共用账户；六个常用分类、按需加载历史邮件、纯文字正文、离线已读队列。不发送或删除邮件。
...
产物：`dist/TodoClock-0.1.6.zip`、对应 SHA256 文件。打包脚本仅包含明确列出的代码、示例配置和文档，不包含本地配置、令牌、待办、日志或设备恢复记录。
...
0.1.6 设置收为两页：第一页包含所有时间周期与退出插件，第二页包含微软账户和合并队列。队列按钮区分“等待同步 N”和“需处理 M”。本次额外仿真及视觉检查使用 `tools/release_016_ui_smoke.py`、`tools/release_016_acceptance.py`，当前预览位于 `preview/0.1.6/`；该例外不改变后续验收职责。
