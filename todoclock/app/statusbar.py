...
def sync_ready(app):
    ...
    if not todo_time or not weather_time:
        return False
    if any(name in app.jobs or name in app.errors for name in ('todo', 'weather', 'flush')):
        return False
    if app.outbox or app.weather_view['errors'] or any(x.get('error') for x in app.todo.get('lists', [])):
        return False
    for name, fetched in (('todo', todo_time), ('weather', weather_time)):
