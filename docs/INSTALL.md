...
解压 `TodoClock-0.1.5.zip`，将其中 `todoclock` 文件夹复制到 Kindle 的 `extensions`，使路径为：
...
4. 添加 Microsoft Graph **委托权限** `Tasks.ReadWrite`；使用邮箱还需 `Mail.ReadWrite` 和 `User.Read`。程序还请求 `offline_access`，用于刷新令牌。
...
退出插件后合并覆盖 `TodoClock-0.1.5.zip`，保留 `config/local.json`、`config/secrets.json`、`state/` 和字体。无需修改 Kindle 系统或启动项。升级后设置共有三页，实体键和页码按钮都能切换。

1. 在原微软应用注册中加入 Graph 委托权限 `Mail.ReadWrite`、`User.Read`，保留 `Tasks.ReadWrite` 与公共客户端流程。
2. 进入“设置 → 第 3 页 → 补充授权 / 登录”，用手机或电脑访问屏幕上的微软网址并输入验证码。
3. 选择与待办相同的微软个人账户，确认新增权限。邮箱和待办共用一个账户；不需要邮箱密码、应用密码或客户端密钥。
4. 授权成功后点击左侧“邮箱”，再点击“同步邮箱”。首次未授权时不会不断请求邮箱服务。

旧版登录可补充授权。程序先核对原账户身份，成功且权限齐全后才保存新令牌；取消、拒绝、权限不全或账户不一致会保留原会话。部分旧令牌无法提供可核对身份时，会明确要求先提交待办队列，再注销并重新从邮箱页登录，不猜测账户归属。验证码窗口的“关闭”用于取消授权；取消期间已完成的授权结果以实际完成时间为准。
...
正文页可以取消待提交操作。已在网络中提交的操作可能已经生效，取消不会把云端邮件改回未读。邮件被删除或权限发生变化时显示失败，不无限重试；可在正文取消，或在“设置第 3 页 → 已读操作”清除失败项。

### 同步周期与错误

默认每 15 分钟同步。设置第 3 页点击周期，依次切换：仅手动、5、15、30、60、120 分钟，重启后保留。仅手动关闭定时拉取，仍允许翻阅时按需加载和联网后提交已读队列。

“同步邮箱”刷新六个分类的首批概览，各自记录成功时间。部分分类失败时保留旧缓存并显示错误，不表示整个邮箱已同步成功。顶部原有勾号仍只表示待办和天气；邮箱以自己的同步状态为准。
